...
function parseModuleFile(filePath) {
  ...
  const sourceFileMatch = src.match(/source: STAR WebAPI (\S+)\)/);
...
const { isError, message, result } = await star.${clientProp}.${exampleOp.jsName}(${exampleArgs});
...
const indexRows = moduleSummaries
  .map((m) => `| [\`star.${m.clientProp}\`](modules/${m.moduleName}.md) | \`${m.routePrefix}\` | ${m.opCount} |`)
  .join('\n');

const totalOps = moduleSummaries.reduce((n, m) => n + m.opCount, 0);

const indexContent = `# WEB5 STAR API — JavaScript SDK Reference
