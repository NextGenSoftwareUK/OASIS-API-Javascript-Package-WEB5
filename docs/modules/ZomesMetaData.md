...
```js
const { isError, message, result } = await star.zomesMetaData.cloneZomeMetaData({
    id: '<id>',
...
```js
const { isError, message, result } = await star.zomesMetaData.createZomeMetaData({});
if (isError) throw new Error(message);
console.log(result);
```
...
Body type: `CreateZomeMetaDataRequest`

| Field | Type |
| --- | --- |
| `Name` | `string` |
| `Description` | `string` |
| `HolonSubType` | `ZomeType` |
| `SourceFolderPath` | `string` |
| `CreateOptions` | `ISTARNETCreateOptions<ZomeMetaDataDNA, STARNETDNA>?` |
...
```js
const { isError, message, result } = await star.zomesMetaData.createZomeMetaDataWithOptions({
    name: "example string",
...
```js
const { isError, message, result } = await star.zomesMetaData.deactivateZomeMetaData({
    id: '<id>',
    version: 1
  });
if (isError) throw new Error(message);
...
```js
const { isError, message, result } = await star.zomesMetaData.deleteZomeMetaData({
    id: '<id>'
  });
if (isError) throw new Error(message);
console.log(result);
...
Body type: `DownloadZomeMetaDataRequest`
...
```js
const { isError, message, result } = await star.zomesMetaData.downloadZomeMetaData({
    id: '<id>',
...
Body type: `EditZomeMetaDataRequest`
...
```js
const { isError, message, result } = await star.zomesMetaData.editZomeMetaData({
    id: '<id>',
...
```js
const { isError, message, result } = await star.zomesMetaData.getAllZomesMetaData({});
if (isError) throw new Error(message);
console.log(result);
```
...
```js
const { isError, message, result } = await star.zomesMetaData.getZomeMetaData({
    id: '<id>'
  });
if (isError) throw new Error(message);
console.log(result);
...
```js
const { isError, message, result } = await star.zomesMetaData.getZomeMetaDataVersions({
    id: '<id>'
  });
if (isError) throw new Error(message);
console.log(result);
...
```js
const { isError, message, result } = await star.zomesMetaData.loadAllZomeMetaDataForAvatar({});
if (isError) throw new Error(message);
console.log(result);
```
...
```js
const { isError, message, result } = await star.zomesMetaData.loadZomeMetaDataFromPath({
    path: 'example string'
  });
if (isError) throw new Error(message);
console.log(result);
...
```js
const { isError, message, result } = await star.zomesMetaData.loadZomeMetaDataFromPublished({
...
```js
const { isError, message, result } = await star.zomesMetaData.loadZomeMetaDataVersion({
    id: '<id>',
    version: '<version>'
  });
if (isError) throw new Error(message);
...
```js
const { isError, message, result } = await star.zomesMetaData.publishZomeMetaData({
    id: '<id>',
...
```js
const { isError, message, result } = await star.zomesMetaData.republishZomeMetaData({
    id: '<id>',
    version: 1,
    sourcePath: "example string",
    launchTarget: "example string",
...
```js
const { isError, message, result } = await star.zomesMetaData.searchZomesMetaData({
...
```js
const { isError, message, result } = await star.zomesMetaData.unpublishZomeMetaData({
    id: '<id>',
    version: 1
  });
if (isError) throw new Error(message);
...
```js
const { isError, message, result } = await star.zomesMetaData.updateZomeMetaData({
    id: '<id>'
  });
if (isError) throw new Error(message);
console.log(result);
