...
```js
import { STARClient } from 'https://esm.sh/@oasisomniverse/web5-api@1.0.2';
const client = new STARClient({ baseUrl: 'https://api.web5.oasisomniverse.one' });
```
...
const client = new STARClient({ baseUrl: '...', fetchImpl: fetch });
```
